...
class _ViewPageState extends State<ViewPage> {
  ...
  List<Map<String, dynamic>> _requirementsData = [];
  List<Map<String, dynamic>> _timeofarrData = [];
  List<Map<String, dynamic>> _responderData = [];

  TextEditingController _idController = TextEditingController();

  Future<void> _getResponderData() async {
  final response = await Supabase.instance.client
      .from('responder')
      .select('i_id, username, time'); // Add .execute() to run the query

  setState(() {
    _responderData = List<Map<String, dynamic>>.from(response);
  });
}
            ...
            children: [
              ...
              Table(
                ...
                children: [
                  ...
                  for (final row in _responderData)
                    TableRow(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(row['i_id'].toString()),
                        ),
                        Padding(
                          padding: const EdgeInsets.all(8.0),
              ...
              // Your existing code to display the rest of the data
              SizedBox(height: 10),
              for (final rowData in _breathingData)
                Card(
                  child: Padding(
